...
public class ConcreteWorkflowMonitor implements WorkflowMonitor {
	...
	public ConcreteWorkflowMonitor() throws SAXException, JAXBException {
		...
		// - Workflows - //
		System.out.println("In the document there are "+root.getWorkflow().size()+" workflows");
		...
		for( Actors dep : root.getActors() ) {
			System.out.println("In the department there are "+dep.getActor().size()+" actors");
			for( Actors.Actor xmlActor : dep.getActor() ) {
				Actor a = new Actor(xmlActor.getName(), xmlActor.getRole());
				actors.put(a.getName(), a);
			}
			System.out.println(actors.size()+" actors were created.");
		}
		
		// - Processes - //
		System.out.println("In the document there are "+root.getProcess().size()+" processes");
	 ...
	 * 
	 * @throws JAXBException If it is not possible to create the {@link JAXBContext} or XML file.
	 * @throws SAXException If a SAX error occurs during parsing the XML Schema.
	 */
	private WorkflowManager unmarshallDocument(File inputFile) throws JAXBException, SAXException, IllegalArgumentException {
